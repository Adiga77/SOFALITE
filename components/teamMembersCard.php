                <h3 class="text-center mb-4" ><strong>Team Members</strong></h3>
				<!-- team members -->
				<div class="row  mb-5 g-3">
						<!--team member 1 -->
						<div class="col-md-4">
							<div class="card w-100 h-100">
								<img src="assets/images/team-members/michael.jpeg" class="card-img-top" alt="...">
								<div class="card-body text-center bg-light text-dark">
									<h5 class="card-title">Team member 1</h5>
									<p class="lead">Adokwu Michael Adiga</p>
								</div>
							</div>
						</div>
						<!--team member 2 -->
						<div class="col-md-4">
							<div class="card w-100 h-100">
								<img src="assets/images/team-members/onuh.jpg" class="card-img-top" alt="...">
								<div class="card-body text-center bg-light text-dark">
									<h5 class="card-title">Team member 2</h5>
									<p class="lead">Onuh Joseph Adakole</p>
								</div>
							</div>
						</div>

						<!--team member 3 -->
						<div class="col-md-4">
							<div class="card w-100 h-100" >
								<img src="assets/images/team-members/innocent.jpg" class="card-img-top" alt="...">
								<div class="card-body text-center bg-light text-dark">
									<h5 class="card-title">Team member 3 </h5>
									<p class="lead">Echono Innocent</p>
								</div>
							</div>
						</div>
					</div>